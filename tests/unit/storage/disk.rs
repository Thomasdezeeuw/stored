...
use stored::storage::{disk, AddError, Storage};

use heph_rt::spawn::options::FutureOptions;
use heph_rt::test::spawn_future;

use crate::util::{block_on, temp_path};
...
fn add_blob() {
    let rt = todo!();
    let path = temp_path();
    let (handle, writer_future) = disk::open(&rt, &path).unwrap();
    spawn_future(writer_future, FutureOptions::default());
    let mut storage = disk::Storage::from(handle);
