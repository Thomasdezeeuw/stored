...
//!
//! It supports three main operations: storing, retrieving and removing blobs.
//! As the key of a blob is its checksum it is not possible to modify blobs. If
//! a blob needs to be modified a new blob simply needs to be stored and the new
//! key used. The client can validate the correct delivery of the blob by using
//! the returned key (checksum). The blob themselves are unchanged by Store*d*.
//!
//! # Code Navigation
//!
...
#![feature(async_iter_from_iter, async_iterator, iter_advance_by, never_type)]
